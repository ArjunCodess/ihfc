...
On boot, the program commands all five servos closed and runs a startup self-check. It verifies unique GPIO assignments, PWM setup, servo commands, one valid ultrasonic echo, the IR input state, and the START input state. A missing ultrasonic echo blocks the mission. The start button must first be released and then held for at least 40 ms. One press starts one mission; reset is required before another run. Sending \texttt{x} through Serial Monitor at 115200 baud stops the motors while USB remains connected. The main switch provides the physical power stop.
