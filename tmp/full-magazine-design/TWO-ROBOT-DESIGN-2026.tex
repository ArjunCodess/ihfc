...
\needspace{12\baselineskip}\section{The design decision}
...
\needspace{12\baselineskip}\section{What the rulebook actually requires}
...
\toprule
\textbf{Task} & \textbf{Required final state and assigned mechanism} & \textbf{Positive points} \\
\midrule\endfirsthead
\toprule\textbf{Task} & \textbf{Required final state and assigned mechanism} & \textbf{Positive points} \\\midrule\endhead
Three samples & SealBot places one 56 mm disc completely inside each of the three 60 mm laboratory slots. & 45 + 5 = 50 \\[3pt]
Two beams & SealBot releases both beams upright, touching the required walls, each other and the boundary tape. & 50 + 20 = 70 \\[3pt]
Ten kits & CareBot empties preloaded batches: six at H, two at left PCC, two at right PCC. & 30 + 20 = 50 \\[3pt]
Twelve patients & Four red to H; four green to RZ; two yellow to each PCC. & 60 + 6 + 8 + 6 = 80 \\[3pt]
Complete positive score & All four rows above. Penalties are separate. & 250 \\[3pt]
...
\begin{center}\includegraphics[width=\linewidth,height=0.40\textheight,keepaspectratio]{C:/Users/USER/Desktop/ihfc/tmp/pdfs/full-design/beam-dimension-check.png}
\par\small Beam and tape dimension conflict in the supplied drawing\end{center}
\needspace{12\baselineskip}\section{Robot layout and legal starting arrangement}
...
\begin{center}\includegraphics[width=\linewidth,height=0.40\textheight,keepaspectratio]{C:/Users/USER/Desktop/ihfc/tmp/pdfs/full-design/start-layout.png}
\par\small Starting envelope, proposed dimensions\end{center}
...
\needspace{12\baselineskip}\section{SealBot: samples before closure}
...
\needspace{10\baselineskip}\subsection{Collect one sample deliberately}
...
\needspace{10\baselineskip}\subsection{Place the samples into three distinct slots}
...
\needspace{10\baselineskip}\subsection{Place the beams without lifting them}
...
\needspace{12\baselineskip}\section{CareBot: collect once, deliver by destination}
...
\needspace{10\baselineskip}\subsection{A magazine students can make}
...
\begin{center}\includegraphics[width=\linewidth,height=0.40\textheight,keepaspectratio]{C:/Users/USER/Desktop/ihfc/tmp/pdfs/full-design/patient-magazine.png}
\par\small Patient magazine and transfer sequence\end{center}
...
\needspace{10\baselineskip}\subsection{Lift and inspect each patient}
...
\needspace{10\baselineskip}\subsection{Deliver all kits with three gates}
...
\needspace{10\baselineskip}\subsection{Deliver patients without losing the yellow split}
...
\needspace{12\baselineskip}\section{ESP32-CAM perception that fits the hardware}
...
Use Espressif's camera driver at QVGA JPEG with one frame buffer, then decode locally for a small region of interest. A full 320 x 240 RGB888 image occupies 230,400 bytes, so the camera board handles it in PSRAM; the Nano receives only small result messages. Espressif warns that direct RGB/YUV capture stresses memory, especially with Wi-Fi. Disable Wi-Fi and Bluetooth for normal match operation. See the \href{https://github.com/espressif/esp32-camera}{official camera driver}.
...
\needspace{12\baselineskip}\section{Routes, field coverage and the two-minute limit}
...
\needspace{10\baselineskip}\subsection{Timing model, not a measured run}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Robot phase} & \textbf{Proposed timing allowance} & \textbf{What must be measured} \\
\midrule\endfirsthead
\toprule\textbf{Robot phase} & \textbf{Proposed timing allowance} & \textbf{What must be measured} \\\midrule\endhead
SealBot samples & 0-48 seconds & Three complete acquisition, transport and slot-placement cycles, including rescans. \\[3pt]
SealBot beams & 48-85 seconds & Two pickups from start, transport, alignment and full release. \\[3pt]
SealBot finish & 85-100 seconds & Joint inspection, one correction if possible, then park clear. \\[3pt]
CareBot collect & 0-65 seconds & Exit, travel to twelve targets, inspection and transfer into twelve pockets. \\[3pt]
CareBot healthcare & 65-95 seconds & Three stops, all kit batches, four red and four yellow patients. \\[3pt]
CareBot recovery & 95-112 seconds & Right-side return, clear-entry check and four green releases. \\[3pt]
Both & 112-120 seconds & Margin and automatic final stop. No new long task. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{12\baselineskip}\section{Electronics, power and pin allocation}
...
The \href{https://www.ti.com/lit/ds/symlink/drv8833.pdf}{TI DRV8833 datasheet} gives package-dependent current limits. Inspect the actual module and test loaded starts, turns and a brief controlled obstruction. If its current or thermal capability is insufficient, buy a suitable driver from reserve before running repeated stalls. Driver sleep must default low during boot.

\needspace{30\baselineskip}\subsection{Pin allocation: shared Nano connections}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Nano pins} & \textbf{Function} & \textbf{Constraint} \\
\midrule\endfirsthead
\toprule\textbf{Nano pins} & \textbf{Function} & \textbf{Constraint} \\\midrule\endhead
D0 / D1 & Camera UART & Disconnect the link for Nano uploads; no debug text mixed into the protocol. \\[3pt]
D2 / D3 & Left/right wheel encoders & Interrupt inputs; single-channel direction comes from the commanded motion. \\[3pt]
D4 / D5 & Left motor input pair & D5 provides PWM. Implement forward/reverse using the driver's truth table. \\[3pt]
D7 / D6 & Right motor input pair & D6 provides PWM; do not change Timer0 timing. \\[3pt]
A0 / A1 / A2 & Left/centre/right line outputs & Calibrate black and white on the actual surface. \\[3pt]
D12 & Start input & Debounced, one activation per power cycle. \\[3pt]
D13 & DRV8833 sleep/enable & External pulldown; enable only after successful boot checks. \\[3pt]
\bottomrule\end{longtable}\normalsize
\needspace{30\baselineskip}\subsection{Pin allocation: CareBot additions}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Pins or channels} & \textbf{Function} & \textbf{Constraint} \\
\midrule\endfirsthead
\toprule\textbf{Pins or channels} & \textbf{Function} & \textbf{Constraint} \\\midrule\endhead
D8-D11 & ULN2003 IN1-IN4 & Non-blocking half-step scheduler. \\[3pt]
A4 / A5 & PCA9685 SDA / SCL & Short I2C wiring, suitable pullups and separate servo power. \\[3pt]
PCA channels 0-2 & H, left PCC, right PCC kit gates & Pre-counted whole batches. \\[3pt]
PCA channel 3 & MG90S patient lift & Calibrated stops; cup stays level. \\[3pt]
PCA channel 4 & Patient cup floor & Closed during pickup and lift. \\[3pt]
PCA channel 5 & Magazine outlet gate & Closed during indexing and loading. \\[3pt]
PCA channel 6 & Camera tilt & Acquisition and inspection stops. \\[3pt]
A3 & Front bumper & Normally closed contact where practical; open circuit stops motion. \\[3pt]
A6 & Cup presence & Analog-only Nano input; external bias resistor. \\[3pt]
A7 & Magazine index sensor & Analog-only input; distinguish home from ordinary station marks. \\[3pt]
\bottomrule\end{longtable}\normalsize
\needspace{30\baselineskip}\subsection{Pin allocation: SealBot additions}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Pins} & \textbf{Function} & \textbf{Constraint} \\
\midrule\endfirsthead
\toprule\textbf{Pins} & \textbf{Function} & \textbf{Constraint} \\\midrule\endhead
D8 & MG90S sample lift & Servo library; keep motor PWM on D5/D6. \\[3pt]
D9 & Sample cup floor servo & Mechanically closed for transport. \\[3pt]
D10 & Beam clamp servo & Low padded contact; opens fully before withdrawal. \\[3pt]
D11 / A3 & Left/right contact switches & Distinguish wall alignment from a one-sided impact. \\[3pt]
A4 & Sample presence switch & Verify occupancy as well as camera detection. \\[3pt]
A5 & Clamp closed switch & Confirms mechanism state, not necessarily beam capture. \\[3pt]
A6 & Battery divider & Analog input; scale full-charge voltage safely below 5 V. \\[3pt]
A7 & Spare analog input & Available for a lift limit or extra presence check. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{12\baselineskip}\section{Control logic and failure recovery}
...
SealBot's normal sequence is START, FIND\_SAMPLE, CAPTURE, VERIFY\_CAPTURE, GO\_LAB, ALIGN\_EMPTY\_SLOT, RELEASE, VERIFY\_SLOT, repeat three times, FETCH\_BEAM, PLACE\_BEAM, repeat twice, CHECK\_CLOSURE, PARK. CareBot's is START, FIND\_PATIENT, CAPTURE, CLASSIFY, INDEX\_EMPTY, TRANSFER, VERIFY\_TRANSFER, repeat twelve times, DELIVER\_LEFT, DELIVER\_H, DELIVER\_RIGHT, RETURN\_RZ, DELIVER\_GREEN, PARK.
...
\clearpage
\needspace{12\baselineskip}\section{Budget: Rs 9,910 for both robots}
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Item} & \textbf{Quantity and allowance} & \textbf{Total Rs} \\
\midrule\endfirsthead
\toprule\textbf{Item} & \textbf{Quantity and allowance} & \textbf{Total Rs} \\\midrule\endhead
Complete 2WD chassis sets & 2 x 350; four motors, wheels, casters and hardware & 700 \\[3pt]
Nano CH340 boards & 2 x 200 & 400 \\[3pt]
ESP32-CAM with OV2640 & 2 x 720, including 18 percent GST on Rs 610 & 1440 \\[3pt]
USB-to-TTL programmer & 1 shared & 150 \\[3pt]
DRV8833 modules & 2 x 80; verify actual current capability & 160 \\[3pt]
SG90 positional servos & 10 x 100; eight installed, two spare & 1000 \\[3pt]
MG90S positional servos & 2 x 200; one lift on each robot & 400 \\[3pt]
28BYJ-48 plus ULN2003 & 1 set, allowance includes matching driver & 200 \\[3pt]
PCA9685 board & 1 for CareBot & 260 \\[3pt]
Five-channel line arrays & 2 x 190; use three outputs each & 380 \\[3pt]
Wheel encoder sensors and discs & Four complete sensing positions & 200 \\[3pt]
Contact and magazine sensors & Switches, optical index sensor, flags & 160 \\[3pt]
Protected 2S battery packs & 2 x 600; confirm discharge rating & 1200 \\[3pt]
Compatible 8.4 V CC/CV charger & 1 shared; correct connector and polarity & 650 \\[3pt]
Buck regulator modules & 6 x 60; three rails per robot & 360 \\[3pt]
Electrical consumables & Fuse, stops, wire, connectors, perfboard, LEDs, capacitors, resistors & 450 \\[3pt]
Mechanism materials and cutting & Sheet, tubes, axle, spacers, PET, fasteners and local cutting & 650 \\[3pt]
Practice pieces and tape & Borrow full-size base and tools & 350 \\[3pt]
Shipping allowance & Consolidated deliveries & 400 \\[3pt]
Repair reserve & Available only after the complete base purchase & 400 \\[3pt]
Total & Both robots, practice consumables, shipping and reserve & 9910 \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{10\baselineskip}\subsection{Price evidence and substitutions}
The \href{https://directronics.in/product/esp32-cam-wifi-module-camera-module}{Directronics camera listing} lists Rs 610 excluding 18 percent GST, which is Rs 719.80 per camera. The \href{https://probots.co.in/2wd-clear-acrylic-smart-robot-chassis-car-kit.html}{Probots chassis listing} supplied the approximately Rs 349 chassis reference. Check the delivered wheel envelope and encoder compatibility.

The \href{https://robocraze.com/collections/servo-motors-controllers}{Robocraze servo catalogue} showed SG90 around Rs 95, positional MG90S around Rs 149 and PCA9685 around Rs 255. Allowances are rounded above these references. Its \href{https://robocraze.com/products/5v-stepper-motor}{28BYJ-48 page} showed Rs 83, but the package text and title differ about the driver; budget Rs 200 and explicitly verify that ULN2003 is included.

The \href{https://www.tomsonelectronics.com/products/5-channel-line-tracking-sensor-module-bfd-1000}{Tomsonelectronics line array} supplied the Rs 189 reference; the \href{https://hubtronics.in/lm393-speed-measuring-sensor-module}{Hubtronics encoder sensor} supplied the approximately Rs 40 sensor-only reference. The budget adds allowance for discs. The \href{https://www.ktron.in/product/lm2596-step-down-module/}{Ktron LM2596 listing} supplied the sub-Rs-60 regulator reference.
...
\clearpage
\needspace{12\baselineskip}\section{Nine-day build sequence}
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Day} & \textbf{Work} & \textbf{Evidence required before moving on} \\
\midrule\endfirsthead
\toprule\textbf{Day} & \textbf{Work} & \textbf{Evidence required before moving on} \\\midrule\endhead
1 & Reproduce the rulebook field; cardboard packing; magazine and sample bench mock-ups; order locally available parts. & Both robots and all initial pieces fit the legal box; horizontal laboratory and beam dimensions reproduced faithfully. \\[3pt]
2 & Finish magazine support/indexing and sample lift/floor; test cameras on actual paint; receive electronics. & Twenty consecutive load/index/unload cycles without lost inventory; twenty sample releases into one slot. \\[3pt]
3 & Wire safe power rails and chassis; calibrate motors, encoders and line sensors; build rear clamp. & Loaded starts and turns without resets; ten beam releases upright and unsupported. \\[3pt]
4 & Add cup transfer, camera tilt and all three kit chutes; make a full-size taped practice half. & 6/2/2 kit drops repeated correctly; test all twelve pockets loaded, not one empty rotor. \\[3pt]
5 & Integrate independent complete robot routes, initially without the other robot. & Measured sample cycle, twelve-patient collection time, loaded magazine sweep time and route length. \\[3pt]
6 & Complete every mission on both robots; shorten approaches and fix mechanical jams. & One autonomous full-task run per robot inside its 120-second limit. \\[3pt]
7 & Run both together with random colour orders and difficult sample placements. & No route conflicts; correct 2/2 yellow split; all three distinct laboratory slots used. \\[3pt]
8 & Repeat at least ten full rounds, alternating lighting and battery state; calculate both penalty interpretations. & Log actual outcomes, failure causes and time remaining; no unverified claim of reliability. \\[3pt]
9 & Repair only demonstrated issues, secure wires, add SDG3 decoration, pack spares and printed setup sheet. & Final loaded inspection, stop test and reproducible pre-match checklist. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\clearpage
\needspace{12\baselineskip}\section{Failure audit and acceptance tests}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Failure exposed by the architecture} & \textbf{Design response} & \textbf{Acceptance check} \\
\midrule\endfirsthead
\toprule\textbf{Failure exposed by the architecture} & \textbf{Design response} & \textbf{Acceptance check} \\\midrule\endhead
Camera sees colour but robot misses the patient & Separate scene acquisition from presence-confirmed cup inspection. & Test offsets, both rows and all allowed site arrangements. \\[3pt]
Two objects enter together & Narrow patient cup and sample cup roof; verify occupancy. & Adjacent pieces and touching samples do not create a false single pickup. \\[3pt]
Colour changes with lighting & Diffused LEDs, repeatable camera stop, calibrated ranges and UNKNOWN. & Test unseen paint samples or orientations under two light conditions. \\[3pt]
Rotor misses a step or jams & Supported axle, real station marks, one-direction final approach, transfer interlock. & All twelve pockets loaded; repeated complete rotations and selective drops. \\[3pt]
Wrong pocket inventory & Commit only after verified transfer or release. & Deliberately block one transfer; no later patient is mislabelled. \\[3pt]
Samples miss their holes & Close camera alignment, measured centres, short vertical drop. & Twenty placements at each slot; include corner sample acquisitions. \\[3pt]
Beam tips during travel & Force near the floor, low acceleration, full-width route clearance. & Real-weight beams on the actual surface, including turns and release. \\[3pt]
Beam joint cannot satisfy tape geometry & Full-size wall/tape jig; keep geometric conflict visible and target poses configurable. & Record which contact condition fails; never claim closure bonus from an impossible drawing. \\[3pt]
Delivered pieces scatter & Short low chute, stationary release, landing margin. & Loaded batch drops with previously delivered pieces present. \\[3pt]
One robot runs late & Separate normal work areas; visually check shared start access. & Delay SealBot deliberately; CareBot does not collide or push beams. \\[3pt]
Servo current resets camera & Separate regulated rails, short grounds, sequenced movement. & Loaded drive plus worst intended mechanism action at low starting voltage. \\[3pt]
Sensor or program stalls & Timeouts, passive retention and independent stop switch. & Disconnect UART or trigger a bumper; motion stops without blind continuation. \\[3pt]
Full route exceeds the round & Measure each phase and the joint run. & Complete all tasks in less than 112 seconds repeatedly before claiming an eight-second margin. \\[3pt]
\bottomrule\end{longtable}\normalsize
