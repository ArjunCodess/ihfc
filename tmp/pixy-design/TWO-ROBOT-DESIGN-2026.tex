...
\vspace{9mm}
{\Huge\bfseries Two robots, redesigned\par}
\vspace{7mm}
{\Large SealBot and CareBot\par}
\vspace{12mm}
\rule{\linewidth}{1pt}
\vspace{8mm}
{\large Dedicated color vision. Individual patient storage.\par}
{\large Sample delivery before perimeter closure.\par}
\vspace{12mm}
\textbf{SealBot}\par
Collect samples $\rightarrow$ retain in separate trays $\rightarrow$ insert at laboratory
$\rightarrow$ place beams $\rightarrow$ park.\par
\vspace{6mm}
\textbf{CareBot}\par
Deliver kits $\rightarrow$ capture patient $\rightarrow$ inspect under fixed light
$\rightarrow$ store in an addressed pocket $\rightarrow$ deliver by color.\par
\vfill
\color{black}
Engineering concept and build specification\par
...
\tableofcontents
\clearpage
\needspace{6\baselineskip}\section{The recommendation}
...
\needspace{6\baselineskip}\section{Rules that determine the design}
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.34\linewidth}>{\raggedright\arraybackslash}p{0.6\linewidth}@{}}
\toprule
\textbf{Requirement} & \textbf{Design consequence and source} \\
\midrule\endfirsthead
\toprule\textbf{Requirement} & \textbf{Design consequence and source} \\\midrule\endhead
Robots, kits, and beams all start within 480 x 280 mm & Gauge the complete loaded assembly, including decoration and guards. Page 4. \\[3pt]
The match lasts 120 seconds & Reserve delivery time and enforce an independent shutdown deadline. Pages 3 and 18. \\[3pt]
Three discs, diameter 56 mm and thickness 5 mm & Use separate retaining trays rather than a stacked shutter. Pages 8-9, including the drawing. \\[3pt]
Laboratory drawing: 440 x 150 x 3 mm panel, three 60 mm holes & Confirm installed orientation, hole centers, and receiving clearance. Page 9. \\[3pt]
Beams: 250 and 280 mm long, section 60 x 20 mm & Release upright, stable on the narrow base, and completely unsupported. Pages 5 and 9-10. \\[3pt]
Each beam touches a fixed wall, the other beam, and the marked line & Prove the corner joint and approach geometry together. Page 5. \\[3pt]
Ten kits, 25 x 25 x 20 mm & Deliver six to H and two to each PCC. Pages 6 and 10. \\[3pt]
Twelve patients, four per color, 20 x 20 mm & Provide one physical storage pocket per cylinder. Pages 11-12. \\[3pt]
Senior patient arrangement may vary & Scan legal side areas; do not assume only color order changes. Pages 7 and 16. \\[3pt]
Accessible emergency stop, safe edges, secured components, SDG 3 decoration & Include them in the initial packaging and inspection. Page 17. \\[3pt]
\bottomrule\end{longtable}\normalsize
\needspace{5\baselineskip}\subsection{Scoring and unresolved wording}
...
\needspace{6\baselineskip}\section{Why Pixy2 rather than ESP32-CAM}
...
\toprule
\textbf{Option} & \textbf{Work required} & \textbf{Decision} \\
\midrule\endfirsthead
\toprule\textbf{Option} & \textbf{Work required} & \textbf{Decision} \\\midrule\endhead
Pixy2 & Teach signatures, filter detections, calibrate optics, integrate wired results & Recommended for this color-specific task. \\[3pt]
HUSKYLENS & Teach colors, read IDs, validate focus and stability & Sensible if already owned; use its own firmware adapter. \\[3pt]
Classic ESP32-CAM & Develop capture, exposure control, segmentation, rejection, and result messaging & Choose for a constrained budget or existing embedded vision experience. \\[3pt]
ESP32-S3 camera board with PSRAM & Similar custom software, with board-specific integration & My preference if building a new custom ESP camera system. \\[3pt]
Close-range RGB sensor & Control distance and illumination, classify aggregate readings & Adequate after capture, but cannot locate patients during approach. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{5\baselineskip}\subsection{One camera, two observation stages}
...
\needspace{5\baselineskip}\subsection{The acceptance decision}
...
\needspace{6\baselineskip}\section{Common chassis and loaded fit}
...
\needspace{6\baselineskip}\section{SealBot: samples before closure}
\needspace{5\baselineskip}\subsection{Intake, metering, and storage}
...
\needspace{5\baselineskip}\subsection{Adjustable laboratory head}
...
\needspace{5\baselineskip}\subsection{Beam carriage}
...
\needspace{6\baselineskip}\section{CareBot: controlled capture and individual storage}
\needspace{5\baselineskip}\subsection{Medical kits}
...
\needspace{5\baselineskip}\subsection{Capture and lift}
...
\needspace{5\baselineskip}\subsection{Twelve-pocket carousel}
...
\needspace{5\baselineskip}\subsection{Route and reinspection}
...
\needspace{6\baselineskip}\section{Navigation and coordination}
...
\needspace{6\baselineskip}\section{Controller and time budget}
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.51\linewidth}>{\raggedright\arraybackslash}p{0.23\linewidth}@{}}
\toprule
\textbf{Robot} & \textbf{Phase} & \textbf{Initial allowance} \\
\midrule\endfirsthead
\toprule\textbf{Robot} & \textbf{Phase} & \textbf{Initial allowance} \\\midrule\endhead
SealBot & Start, exit, and sample capture & 35 s \\[3pt]
SealBot & Laboratory docking and three insertions & 30 s \\[3pt]
SealBot & Two beam placements & 30 s \\[3pt]
SealBot & Recovery and parking reserve & 23 s \\[3pt]
CareBot & Kit distribution & 30 s \\[3pt]
CareBot & Patient search, capture, inspection, storage & 42 s \\[3pt]
CareBot & Deliveries including RZ return & 30 s \\[3pt]
CareBot & Recovery and parking reserve & 16 s \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{6\baselineskip}\section{Procurement specification}
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.34\linewidth}>{\raggedright\arraybackslash}p{0.6\linewidth}@{}}
\toprule
\textbf{Assembly} & \textbf{Items and selection check} \\
\midrule\endfirsthead
\toprule\textbf{Assembly} & \textbf{Items and selection check} \\\midrule\endhead
Each base & Two encoder motors, wheels, compliant support, driver, MCU. Size from mass and stall current. \\[3pt]
Each navigation set & Reflectance array, obstacle sensing, docking switches. Validate on the actual field. \\[3pt]
Each electrical set & Battery, fuse, emergency stop, master switch, regulators, wiring, guards. Test brownouts. \\[3pt]
CareBot vision & One Pixy2, adjustable mount, diffuse LEDs, shutter, presence sensor. Prove both image regions. \\[3pt]
CareBot patients & Scoop, capture actuator, guided elevator, carousel, index drive, position sensors, support fingers. \\[3pt]
CareBot kits & Three destination magazines, holdback and metering gates, exit sensing. Hospital capacity six. \\[3pt]
SealBot samples & Roller, gate, three-tray indexer, retention sensing, tilt cradle, limited-force pusher. \\[3pt]
SealBot beams & Two padded cassettes, guided carriage, latches, end sensing, stable supports. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{6\baselineskip}\section{Build order and acceptance}
\begin{enumerate}
\item Build the field fixtures and loaded starting gauge. Confirm laboratory installation, legal patient variation, and penalty interpretation. Measure the actual game pieces.
\item Build the camera-and-capture fixture first. Test at least 300 labeled presentations across all cylinders, lighting changes, empty pockets, partial captures, and kit crosses. Target zero wrong accepted colors and no more than five percent UNKNOWN. Zero observed mistakes is not proof of zero future errors.
\item Prototype the carousel handoff. Run 200 mixed loads, indexes, and unloads with no inventory drift, unintended release, or wrong-pocket transfer. Inject a blocked port and missed position signal to test interlocks.
\item Prove the loaded beam carrier. Target at least 48 successful paired closures in 50 trials, with unsupported beams, correct contacts, and no damage or manual correction.
\item Test sample capture on at least 30 varied legal layouts, including edge and corner cases. Prove laboratory insertion separately before combining the route.
\item Tune one drive base and duplicate it. Validate loaded travel, tape reacquisition, turning, low-battery stopping, and clearance throughout mechanism motion.
\item Target at least 19 exact kit distributions in 20 full trials. Then add triage and compare measured pickup and unload cycles with the budget.
\item Run both robots together for at least 20 randomized matches. Include delays, a missed pickup, camera timeout, and blocked corridor. Require zero collisions, safe failure handling, and shutdown before 120 seconds. Record scores using the confirmed rules.
\item Freeze the competition task set from results. If twelve-patient triage does not fit, shorten acquisition and deliver confirmed onboard patients. Disable unreliable subsystems through configuration and recalculate expected score with penalties included.
\end{enumerate}
...
\needspace{6\baselineskip}\section{What changed}
...
\needspace{6\baselineskip}\section{Sources and release measurements}
\begin{itemize}
\item S1. Supplied Rulebook-2026.pdf. Pages 4-17 cover objects, layout, and robots; pages 20-24 cover Senior scores and penalties. Drawings on pages 9-10 and 15-16 were visually inspected.
\item S2. \href{https://docs.pixycam.com/wiki/doku.php?id=wiki:v2:ccc_api}{Pixy2 color connected components API}. Official signature, geometry, tracking, and polling reference. Accessed 5 September 2026.
\item S3. \href{https://www.dfrobot.com/huskylens/huskylens}{DFRobot HUSKYLENS}. Official color-recognition and UART/I2C description. Accessed 5 September 2026.
\item S4. \href{https://github.com/espressif/esp32-camera}{Espressif ESP32 camera driver}. Official processor support, memory requirements, and capture caveats. Accessed 5 September 2026.
\end{itemize}
