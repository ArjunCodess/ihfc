# Two robots on a Rs 7,000 budget

SealBot and CareBot | Nine-day school build | Revised 5 September 2026
...
## 2. What we keep and what we remove

| Previous design fault | Replacement | Why it helps |
|---|---|---|
| Pixy2 consumes more than the total budget | One ESP32-CAM with OV2640 and verified PSRAM | Enough processing for a fixed inspection area, at a small fraction of the cost. |
| Twelve-pocket carousel needs indexing and transfer interlocks | One patient pocket | No inventory map, storage indexing, or mixed-color bin. |
| Elevator creates two extra handoffs | Floor-level shallow scoop | Patient stays in one place from pickup to delivery. |
| Three sample trays and precision laboratory insertion | Omit sample mission for this build | Removes an entire mechanism and its 2 mm nominal radial alignment problem. |
| Stacked or diagonal beam carriage creates fit and unloading problems | Beams start on the floor; a low clamp moves one at a time | No carrier, lifting, gravity slide, or support withdrawal. |
| Metered kit magazines need holdback and counting | Three pre-counted batch chutes | Each destination receives its whole assigned load in one operation. |
| Timed driving drifts with battery and payload | Cheap wheel pulse sensors plus field references | Measures wheel travel and corrects at existing lines and walls. |
| Fixed robot delay assumes nothing jams | Separate working areas, bumpers, and timeout stops | A delayed robot cannot authorize blind movement by the other. |
...
| Item | Quantity / allowance basis | Total Rs |
|---|---|---|
| 2WD kits, motors, wheels, caster, brackets | 2 at 350 | 700 |
| Nano-compatible CH340 boards | 2 at 200 | 400 |
| ESP32-CAM with OV2640 | 1, including tax allowance | 720 |
| USB-to-TTL programmer and programming leads | 1, 3.3 V UART logic | 150 |
| DRV8833 dual motor-driver modules | 2 at 80 | 160 |
| SG90 180-degree servos | 8 at 100; six working, two spare | 800 |
| Five-channel line sensor boards | 2 at 190; initially use three channels | 380 |
| Wheel slot sensors and encoder discs | 4 sensor/disc sets | 200 |
| Preassembled protected 2S Li-ion packs | 2 at 550, about 2 Ah each | 1,100 |
| Compatible 8.4 V CC/CV charger | 1, charge packs sequentially | 600 |
| LM2596-type adjustable regulators | 4 at 60 | 240 |
| Switches, fuses, wire, connectors, perfboard, LEDs, capacitors, resistors | Shared allowance | 350 |
| Deck offcuts, PET sheet, hinges, fasteners, tape, glue, guards | Shared allowance | 250 |
| Practice game pieces and markings | School-cut offcuts; surface borrowed | 150 |
| Delivery / local collection | Consolidated orders | 300 |
| Small repair reserve | In addition to spare servos | 200 |
| Planned total | Both robots and shared supplies | 6,700 |
| Unallocated headroom | Keep below the hard ceiling | 300 |
| Maximum authorized design budget | Do not spend beyond this | 7,000 |
...
## 4. CareBot: kit delivery and one patient at a time
...
### A small scoop with positive release
...
## 5. ESP32-CAM implementation

### Keep the camera job small
...
### Classification that students can debug

1. Capture labeled images from all twelve actual cylinders, several rotations, an empty pocket, partial pickups, and different room lighting. Keep some captures separate for validation instead of tuning on everything.

2. Crop a fixed inspection region. Convert its pixels to HSV or normalized RGB. Reject pixels that are too dark, too bright, or insufficiently saturated. Handle red on both sides of the hue wraparound.

3. Count red, yellow, and green votes using thresholds calibrated on the fixture. As an initial rule, require at least 60 percent usable pixels in the crop and an 80 percent winning share among usable pixels. These thresholds are starting values; the shares are not probabilities of correctness.

4. Require the same accepted color across five fresh frames acquired after the request. Reject empty or stale images. Allow one repeat after settling, then return UNKNOWN. Target completion within one second; cap the whole inspection attempt at two seconds and verify actual timing on Day 2.
...
### Finding a patient is a separate problem
...
## 6. SealBot: collect floor-staged beams one at a time
...
## 7. Electrical and navigation details that prevent avoidable failures

### Two power branches per robot
...
### Pin allocation for CareBot's classic Nano

| Function | Candidate pins | Check |
|---|---|---|
| Camera UART | D0, D1 | Disconnect when programming; level-shift Nano TX. |
| Wheel pulse inputs | D2, D3 | Keep interrupt handlers short. |
| Motor inputs | D4/D5 and D7/D6 | D5 and D6 provide PWM; implement both directions using the driver truth table. |
| Three kit gates | D8, D9, D10 | Servo pulses, not motor PWM. |
| Patient gate and ejector | D11, A3 | Five servo signals total. |
| Start button | D12 | Require a new press after boot. |
| Driver sleep control | D13 | Match the breakout's sleep wiring and keep drive disabled at boot. |
| Three line channels | A0, A1, A2 | Treat selected board outputs according to their actual type. |
| Left and right soft bumper switches | A4, A5 | Dock slowly; unexpected contact stops travel. |
| Pocket presence | A6 | Analog reading with an external resistor; A6 is not a normal digital pin. |
| Battery divider | A7 | Divider keeps full pack voltage within ADC limits. |
...
## 8. Mission control and collision handling
...
1. Day 1: confirm the local rules, existing equipment, and parts availability. Make the starting gauge and beam-release mock-up with actual-size pieces. Order or collect parts immediately. Assign students to mechanics, wiring, and code; keep one shared notebook of dimensions and changes.

2. Day 2: power and flash both Nanos and the camera. Demonstrate red, yellow, green, and UNKNOWN in the fixed pocket. Assemble both drive bases and test power isolation. If parts are still unavailable, reduce the build rather than pretending nine full days remain.

3. Day 3: finish the three batch chutes and the patient gate/ejector. Run 30 stationary releases per chute and 30 patient pickup/release cycles. Correct jams now; do not hide them with longer delays.

4. Day 4: get each robot to repeat basic travel, turns, line reacquisition, and low-speed docking. Complete one beam placement, then work on the second. Verify both loaded robots fit together with cables and decoration.

5. Day 5: complete the kit route and a single patient round trip. Check patient acquisition under the organizer's actual randomization rule. This is the triage decision point: if acquisition or color acceptance is unreliable, stop expanding that feature.

6. Day 6: integrate both robots, set the final route sequence, and add deadline, stall, missing-line, and camera-timeout handling. Freeze mechanisms and parts at the end of the day. No carousel, lift, new camera library, or sample mechanism after this point.

7. Day 7: run at least ten full matches with varied patient colors, lower battery charge, and room-light changes. Log each failure and score. Test one delayed robot and one blocked path. Fix the largest repeated fault first.

8. Day 8: run another ten full matches. Require at least nine correct kit distributions, repeatable unsupported beam placement, no accepted wrong colors, no collisions, and stops before 120 seconds. Disable a failing optional patient routine instead of adding a late workaround.

9. Day 9: rehearse setup, charge packs, label connectors, secure wiring, save known-good firmware, and pack the spare servos and repair materials. Make only calibration adjustments. Do not rebuild the robot the night before the event.
...
## 10. Scoring tradeoffs and remaining risks
...
## 11. Price references and technical sources
...
- P1. [Probots 2WD kit](https://probots.co.in/2wd-clear-acrylic-smart-robot-chassis-car-kit.html): Rs 349 including GST, with motors and caster. Verify encoder discs or supply them separately.
- P2. [Robu Arduino-compatible board listings](https://robu.in/product-category/microcontroller-development-board/arduino-boards/arduino-board/): retrieved listings showed Nano variants around Rs 195-199 including GST. Confirm the selected board and cable.
- P3. [Directronics ESP32-CAM](https://directronics.in/product/esp32-cam-wifi-module-camera-module): Rs 610 before 18 percent GST, or Rs 719.80. Confirm OV2640 and PSRAM; programmer is separate.
- P4. [Robocraze SG90](https://robocraze.com/products/sg90-micro-servo-motor): Rs 91 including GST when inspected. The site announced shipping resumes on 7 September for orders after its cutoff, so local stock may be more useful for this build.
- P5. [Tomson BFD-1000](https://www.tomsonelectronics.com/products/5-channel-line-tracking-sensor-module-bfd-1000): Rs 189 including tax.
- P6. [Hubtronics slot sensor](https://hubtronics.in/lm393-speed-measuring-sensor-module): Rs 40 including GST. Allow separately for discs and mounting.
- P7. [Robodo protected 7.4 V pack](https://robodo.in/products/li-ion-battery-7-4v-2200mah-2c-wth-inbuilt-charger-protection): displayed Rs 427 including GST, but availability text is ambiguous. Budget Rs 550 per protected replacement and confirm specifications and stock locally.
- P8. [Goldstar 8.4 V charger](https://goldstarstore.in/products/8-4v-1a-lithium-battery-charger-made-in-india-laptrust): displayed Rs 550 and a sold-out label. This supports a price allowance only; obtain a compatible available charger or borrow one. Rs 600 is reserved.
- P9. [KTRON LM2596 module](https://www.ktron.in/product/lm2596-step-down-module/): Rs 46.80 including GST. The plan allows Rs 60 each and requires load testing.
- P10. [Robu driver listings](https://robu.in/product-category/dc-motors/motor-drivers/stepper-motor-driver/): retrieved listing showed DRV8833 at Rs 72 including GST. Verify package and load suitability rather than relying on the headline current rating.
- T1. [Espressif camera driver](https://github.com/espressif/esp32-camera): official memory, capture-format, and frame-buffer guidance.
- T2. [TI DRV8833 datasheet](https://www.ti.com/lit/ds/symlink/drv8833.pdf): official drive logic and package-dependent current ratings.
- T3. [Arduino Servo timer allocation](https://github.com/arduino-libraries/Servo/blob/master/src/avr/ServoTimers.h): official AVR timer selection.
- R1. Supplied Rulebook-2026.pdf, pages 4-7 for starting conditions and missions, pages 9-16 for objects and geometry, page 17 for robot requirements, and pages 21-24 for Senior scoring and conflicting penalties.
