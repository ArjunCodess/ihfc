...
\needspace{12\baselineskip}\section{Build the simpler version}
...
\needspace{12\baselineskip}\section{The whole job, divided between two robots}
...
\toprule
\textbf{Robot} & \textbf{What it does} & \textbf{Mechanisms} \\
\midrule\endfirsthead
\toprule\textbf{Robot} & \textbf{What it does} & \textbf{Mechanisms} \\\midrule\endhead
SealBot & Collect three samples, place one in each lab slot, then place both beams and park. & One sample tray; one low beam clamp. \\[3pt]
CareBot & Deliver six kits to H and two to each PCC; then carry patients individually to their colour destinations. & Three batch chutes; one patient pocket with gate and ejector. \\[3pt]
...
\needspace{12\baselineskip}\section{CareBot stays almost unchanged}
\needspace{10\baselineskip}\subsection{Deliver the kits in three batches}
...
\needspace{10\baselineskip}\subsection{Keep one patient in one pocket}
...
\needspace{10\baselineskip}\subsection{Add the missing full patient sequence in software}
...
\needspace{12\baselineskip}\section{SealBot gains one sample tray}
\needspace{10\baselineskip}\subsection{Pick up one disc at a time}
...
\needspace{10\baselineskip}\subsection{Drop it into an empty laboratory slot}
...
\needspace{10\baselineskip}\subsection{Then use the previous beam clamp}
...
\needspace{12\baselineskip}\section{Fit, routes and the two-minute limit}
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.17\linewidth}>{\raggedright\arraybackslash}p{0.51\linewidth}>{\raggedright\arraybackslash}p{0.23\linewidth}@{}}
\toprule
\textbf{Robot} & \textbf{Normal sequence} & \textbf{Timing consequence} \\
\midrule\endfirsthead
\toprule\textbf{Robot} & \textbf{Normal sequence} & \textbf{Timing consequence} \\\midrule\endhead
SealBot & Three sample-to-lab trips; two beam trips; park. & Measure both task groups together. Adding samples changes the earlier beam-only schedule. \\[3pt]
CareBot & Three kit stops; repeat pickup, inspect and direct delivery. & At 35-45 seconds for kits and 15-20 seconds per patient, only a few patients fit. These are planning examples, not test results. \\[3pt]
\bottomrule\end{longtable}\normalsize
...
\needspace{12\baselineskip}\section{Keep the electronics familiar}
...
\toprule
\textbf{Nano pins} & \textbf{CareBot} & \textbf{SealBot} \\
\midrule\endfirsthead
\toprule\textbf{Nano pins} & \textbf{CareBot} & \textbf{SealBot} \\\midrule\endhead
D0 / D1 & Camera UART & Camera UART \\[3pt]
D2 / D3 & Wheel pulse inputs & Wheel pulse inputs \\[3pt]
D4/D5 and D7/D6 & Motor input pairs; PWM on D5/D6 & Same \\[3pt]
D8 / D9 / D10 & Three kit gates & Sample lift / sample floor / beam clamp \\[3pt]
D11 / A3 & Patient gate / ejector & Spare \\[3pt]
D12 / D13 & Start button / driver sleep & Same \\[3pt]
A0 / A1 / A2 & Three line channels & Same \\[3pt]
A4 / A5 & Left/right bumper & Same \\[3pt]
A6 / A7 & Pocket presence / battery divider & Tray presence / battery divider \\[3pt]
\bottomrule\end{longtable}\normalsize
A6/A7 are analog-only on the classic Nano and need appropriate external resistors. Keep motor PWM off D9/D10 because the AVR Servo library uses Timer1. Keep the normal Timer0 timing on D5/D6. Driver sleep must default low during boot. Check the module against the \href{https://www.ti.com/lit/ds/symlink/drv8833.pdf}{TI DRV8833 datasheet}, whose current ratings depend on package.
...
\small\begin{longtable}{@{}>{\raggedright\arraybackslash}p{0.22\linewidth}>{\raggedright\arraybackslash}p{0.42\linewidth}>{\raggedright\arraybackslash}p{0.27\linewidth}@{}}
\toprule
\textbf{Item} & \textbf{Quantity / allowance} & \textbf{Total Rs} \\
\midrule\endfirsthead
\toprule\textbf{Item} & \textbf{Quantity / allowance} & \textbf{Total Rs} \\\midrule\endhead
Complete 2WD chassis kits & 2 x 350 & 700 \\[3pt]
Nano CH340 boards & 2 x 200 & 400 \\[3pt]
ESP32-CAM with OV2640 and PSRAM & 2 x 720 & 1440 \\[3pt]
Shared USB-to-TTL programmer & 1 & 150 \\[3pt]
DRV8833 motor drivers & 2 x 80 & 160 \\[3pt]
SG90 positional servos & 9 x 100; seven working, two spare & 900 \\[3pt]
MG90S positional servo & 1 sample lift & 200 \\[3pt]
Five-channel line arrays & 2 x 190 & 380 \\[3pt]
Wheel pulse sensors and discs & Four sensing positions & 200 \\[3pt]
Protected 2S packs & 2 x 600; verify current rating & 1200 \\[3pt]
Compatible 8.4 V charger & 1 shared & 650 \\[3pt]
Buck regulators & 6 x 60 & 360 \\[3pt]
Electrical consumables & Stops, fuse, presence switches, wire, LEDs, connectors and capacitors & 450 \\[3pt]
Mechanism materials & Offcuts, PET, linkages, fasteners and local cutting & 500 \\[3pt]
Practice pieces and tape & Full-size base and tools borrowed & 300 \\[3pt]
Shipping / local collection & Consolidated orders & 400 \\[3pt]
Repair reserve & In addition to spare servos & 400 \\[3pt]
Planned total & Both complete robots and shared supplies & 8790 \\[3pt]
\bottomrule\end{longtable}\normalsize
...
Price references: \href{https://directronics.in/product/esp32-cam-wifi-module-camera-module}{Directronics camera}, \href{https://probots.co.in/2wd-clear-acrylic-smart-robot-chassis-car-kit.html}{Probots chassis}, \href{https://robocraze.com/collections/servo-motors-controllers}{Robocraze servos}, \href{https://www.tomsonelectronics.com/products/5-channel-line-tracking-sensor-module-bfd-1000}{Tomson line array}, \href{https://hubtronics.in/lm393-speed-measuring-sensor-module}{Hubtronics wheel sensor}, and \href{https://www.ktron.in/product/lm2596-step-down-module/}{KTRON regulator}. Rounded allowances include small substitutions; no orders have been placed.

\needspace{12\baselineskip}\section{Nine days, with the difficult part first}
\begin{enumerate}
\item Day 1: make the legal start-size mock-up and the single sample tray. Reproduce a 56 mm disc and 60 mm lab hole. Check the lift and drop by hand before mounting anything on a chassis. Collect available parts.
\item Day 2: bring up both Nanos and cameras. Demonstrate reliable colour readings in CareBot's pocket and repeated sample drops in SealBot's fixture. Fix the tray now if it jams or misses.
\item Day 3: finish the three kit chutes, patient gate/ejector and low beam clamp. Repeat each release on the bench with real-size pieces. Wire and load-test both power systems.
\item Day 4: calibrate driving, turns, line references and slow docking. Test corner sample pickups and the three distinct lab slots. Check the complete starting arrangement again.
\item Day 5: run SealBot's sample-and-beam sequence and CareBot's kit route plus one patient trip. Record actual times. Add the remaining sticker visits and yellow counters using the same repeated routine.
\item Day 6: run both robots together. Fix overlapping approaches and add missing-line, camera-timeout, stalled-movement and deadline stops. Freeze mechanisms at the end of the day.
\item Day 7: run full two-minute rounds with random patient colours and different sample positions. Fix the largest repeated failure; do not add new hardware.
\item Day 8: repeat full rounds at different battery levels and lighting. Record final pieces and score, not merely completed waypoints. Select a match sequence supported by those results.
\item Day 9: rehearse loading and start, charge packs, secure wiring, add health/SDG3 decoration, save working firmware and pack spares. Make calibration changes only.
\end{enumerate}
