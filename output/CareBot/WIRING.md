...
| `D35` | `IO35` or `35` | 35 | Left | Left rear IR digital output, at a compatible logic voltage |
| `RX2` | `IO16` or `16` | 16 | Right | Right rear IR digital output, at a compatible logic voltage |
| `D21` | `IO21` or `21` | 21 | Right | PCA9685 SDA |
| `D22` | `IO22` or `22` | 22 | Right | PCA9685 SCL |
| `D32` | `IO32` or `32` | 32 | Left | Start button; other terminal to GND |
...
On boot/reset, both CareBot sketches detect the driver, configure it for 50 Hz, and command channels 0 through 4 closed. Confirm the actual servo socket numbers against the table above. In the full `CareBotESP32.ino` sketch, with empty mechanisms, send `0` through `9` or `A` through `F` at 115200 baud to test one PCA9685 socket while the motors remain off. `A` means channel 10 and `F` means channel 15. During the mission, the configured servos only open when their delivery step is reached; reset closes them again. A missing PCA9685 or failed I2C command prevents the mission from starting.
