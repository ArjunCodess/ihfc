...
- Program storage: **295,535 bytes**, 22% of the selected partition.
...
`tests/test.cpp` includes the actual packaged sketch using a relative path. `tests/Arduino.h` provides simulated GPIO, time, ultrasonic echoes and Serial input. That header is deliberately outside the sketch folder; it is only for host tests and must not be copied into the Arduino sketch.
